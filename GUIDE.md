...
```bash
mysql -u root -p < supabase/schema.mariadb.sql
```
...
2. Buat user baru (misalnya `ayp_user`) dan berikan hak akses penuh ke database `ayp_affiliate`:
   ```sql
   CREATE USER 'ayp_user'@'localhost' IDENTIFIED BY 'your_dev_db_password';
   GRANT ALL PRIVILEGES ON ayp_affiliate.* TO 'ayp_user'@'localhost';
   FLUSH PRIVILEGES;
   EXIT;
   ```
